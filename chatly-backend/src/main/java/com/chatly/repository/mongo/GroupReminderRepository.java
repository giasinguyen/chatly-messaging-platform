...
public interface GroupReminderRepository extends MongoRepository<GroupReminder, String> {
    ...
    List<GroupReminder> findByRemindAtBeforeAndCompletedFalseAndNotifiedFalse(Instant now);

    List<GroupReminder> findByRemindAtBeforeAndCompletedTrue(Instant now);
}
