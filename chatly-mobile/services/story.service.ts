...
export const storyService = {
  create: async (payload: StoryCreationRequest): Promise<ApiResponse<StoryResponse>> => {
    const response = await axiosClient.post<ApiResponse<StoryResponse>>('/api/stories', payload);
  ...
  reactToStory: async (
    storyId: string,
    emoji: string
  ): Promise<ApiResponse<StoryReactionResponse>> => {
  ...
  replyToStory: async (
    storyId: string,
    content: string
  ): Promise<ApiResponse<StoryReplyResponse>> => {
