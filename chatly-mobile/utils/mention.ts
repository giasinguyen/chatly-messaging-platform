...
export function buildMentionSuggestions(
  ...
  if (includeAll && 'all'.startsWith(normalizedQuery)) {
    results.push({
      id: 'all',
  ...
  if (includeAi && 'ai'.startsWith(normalizedQuery)) {
    results.push({
      id: 'AI',
      displayName: 'AI',
      username: 'AI',
...
export function insertMentionAtCursor(
  ...
  const userMentionField = options.userMentionField ?? 'username';
...
export function extractMentionTargets(
  ...
  const includeAll = options.includeAll ?? false;
  const parts = [...candidates.flatMap((candidate) => [candidate.displayName, candidate.username])];
  ...
  const uniqueParts = [...new Set(parts.filter(Boolean))].sort((left, right) => right.length - left.length);
  if (!uniqueParts.length) {
    return [];
  }

  const mentionRegex = new RegExp(`@(${uniqueParts.map(escapeRegExp).join('|')})`, 'gi');
