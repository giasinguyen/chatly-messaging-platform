...
import type { StoryGroup, StoryReactionResponse, StoryReplyResponse, StoryResponse } from '@/types/story';
...
export function StoryViewerModal({
    ...
    useEffect(() => {
        clearTimer();
        if (isPaused || !currentStory || isOwnerPanelOpen) return;
    ...
    useEffect(() => {
        ...
        storyService.recordView(currentStory.id)
    ...
    const loadPanelData = useCallback(async (tab: OwnerPanelTab, forceReload = false) => {
        if (!currentStory?.id) return;
    ...
    const handleOpenOwnerPanel = useCallback((tab: OwnerPanelTab) => {
        setOwnerTab(tab);
    ...
    const handleTabChange = useCallback((tab: OwnerPanelTab) => {
        setOwnerTab(tab);
        void loadPanelData(tab);
    }, [loadPanelData]);
        ...
        >
            ...
            >
                ...
                <View className="flex-1 relative">
                    ...
                    {/* Header */}
                    <View className="absolute top-16 left-0 right-0 flex-row items-center justify-between px-4 z-10">
                        <View className="flex-row items-center gap-3">
                            <View className="w-9 h-9 rounded-full overflow-hidden bg-white/20">
                                {user.avatarUrl ? (
                                    <Image
                    ...
                    {currentStory.musicName && !isOwnerPanelOpen && (
                        <View className="absolute bottom-28 left-4 right-4 z-10 flex-row items-center gap-2 bg-black/40 rounded-full px-3 py-2">
                    ...
                    {isOwnStory && !isOwnerPanelOpen && (
                        <TouchableOpacity
                            ...
                            className="absolute bottom-6 left-4 right-4 z-10 flex-row items-center gap-2 bg-black/50 rounded-full px-4 py-3"
                            activeOpacity={0.8}
                        >
                            ...
                            <Text className="text-white/50 text-xs">Tap to see</Text>
                        </TouchableOpacity>
                    )}
                    ...
                    {!isOwnStory && (
                        <View className="absolute bottom-6 left-4 right-4 z-10 gap-2">
                                ...
                                {QUICK_EMOJIS.map((emoji) => (
                                    <TouchableOpacity
                                        ...
                                        style={{
                                            padding: 8,
                                            borderRadius: 999,
                                            backgroundColor: myReaction === emoji ? 'rgba(255,255,255,0.3)' : 'rgba(0,0,0,0.4)',
                            ...
                            {/* Reply input */}
                            <View className="flex-row items-center bg-black/50 rounded-full px-4 py-2 gap-2">
                                <TextInput
                                    value={replyText}
                    ...
                    {/* Owner panel */}
                    {isOwnStory && isOwnerPanelOpen && (
                        <View className="absolute bottom-0 left-0 right-0 z-20 bg-black/85 rounded-t-2xl max-h-72">
                            {/* Tabs */}
                            ...
                            <View className="border-b border-white/10 mx-4 mt-0" />
                            ...
                            {isPanelLoading ? (
                                ...
                                <View style={{ maxHeight: 200, overflow: 'hidden' }}>
                                    {ownerTab === 'viewers' && (
                                        viewers.length === 0 ? (
                                            <Text className="text-white/40 text-sm text-center py-6">No viewers yet</Text>
                                        ) : (
                                            viewers.map((v) => (
                                                <View key={v.id} className="flex-row items-center gap-3 px-4 py-2">
                                                    <AvatarBubble name={v.displayName ?? v.username} uri={v.avatarUrl} />
                                                    <Text className="text-white text-sm">{v.displayName ?? v.username}</Text>
                                                </View>
                                            ))
                                        )
                                    )}
                                    ...
                                    {ownerTab === 'reactions' && (
                                        reactions.length === 0 ? (
                                            <Text className="text-white/40 text-sm text-center py-6">No reactions yet</Text>
                                        ) : (
                                            reactions.map((r) => (
                                                <View key={r.id} className="flex-row items-center gap-3 px-4 py-2">
                                                    <AvatarBubble name={r.user?.displayName ?? r.user?.username ?? '?'} uri={r.user?.avatarUrl} />
                                                    <Text className="text-white text-sm flex-1">{r.user?.displayName ?? r.user?.username}</Text>
                                    ...
                                    {ownerTab === 'replies' && (
                                        replies.length === 0 ? (
                                            <Text className="text-white/40 text-sm text-center py-6">No replies yet</Text>
                                        ) : (
                                            replies.map((r) => (
                                                <View key={r.id} className="flex-row items-start gap-3 px-4 py-2">
                                                    <AvatarBubble name={r.user?.displayName ?? r.user?.username ?? '?'} uri={r.user?.avatarUrl} />
                                                    <View>
                                                        <Text className="text-white/60 text-xs">{r.user?.displayName ?? r.user?.username}</Text>
                                                        <Text className="text-white text-sm">{r.content}</Text>
                                                    </View>
                                                </View>
                                            ))
                                        )
...
function AvatarBubble({ name, uri }: AvatarBubbleProps) {
    return (
        <View className="w-8 h-8 rounded-full overflow-hidden bg-white/20 shrink-0">
            {uri ? (
                <Image source={{ uri }} style={{ width: '100%', height: '100%' }} contentFit="cover" />
            ) : (
                <View className="flex-1 items-center justify-center">
                    <Text className="text-white text-xs font-bold">{name.charAt(0).toUpperCase()}</Text>
                </View>
            )}
        </View>
    );
