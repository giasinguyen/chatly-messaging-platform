...
export default function SavedPage({ embedded = false }: SavedPageProps) {
    ...
    const loadSavedPosts = useCallback(async (targetPage: number) => {
        ...
        try {
            ...
            if (response.code !== 1000 || !response.result) {
                throw new Error(
                    response.message ?? t("settings.saved_posts.load_failed"),
                );
            }
            ...
            const message =
                loadError instanceof Error
                    ? loadError.message
                    : t("settings.saved_posts.load_failed");
            setError(message);
            if (!isInitial) {
                toast.error(message);
            }
    ...
    return (
        <SocialErrorBoundary
            title={t("settings.saved_posts.unavailable_title")}
            message={t("settings.saved_posts.unavailable_message")}
        >
            <div
                className={cn(
                    ...
                    {isLoading ? (
                        ...
                        <div className="rounded-lg border border-dashed border-border bg-card/60 px-6 py-10 text-center">
                            <p className="text-sm font-semibold text-foreground">
                                {error
                                    ? t("settings.saved_posts.load_failed")
                                    : t("settings.saved_posts.empty_title")}
                            </p>
                            <p className="mt-1 text-sm text-muted-foreground">
                                {error ?? t("settings.saved_posts.empty_hint")}
                            </p>
                            {error && (
                                <Button
                                    variant="outline"
                        ...
                        <div className="flex flex-col gap-4">
                            ...
                            {hasMore && (
                                ...
                                >
                                    ...
                                    {t("settings.saved_posts.load_more")}
                                </Button>
                            )}
                        </div>
                    )}
