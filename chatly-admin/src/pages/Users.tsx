...
import { Search, ShieldAlert, ShieldCheck, Trash2, Mail, Phone, Calendar, Loader2 } from 'lucide-react';
...
export const Users: React.FC = () => {
  ...
  const handleToggleSuspend = async (userId: string, currentStatus: boolean) => {
    ...
    try {
      ...
      if (response.code === 1000) {
        toast.success(targetState ? 'User suspended successfully' : 'User unsuspended successfully');
        setUsers(prev => prev.map(u => u.id === userId ? { ...u, suspended: targetState } : u));
      } else {
  ...
  const handleDeleteUser = async (userId: string, username: string) => {
    if (!confirm(`Are you absolutely sure you want to permanently delete user @${username}? This action is irreversible.`)) {
      return;
    }
    setIsUpdating(userId);
    try {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-800 tracking-tight font-outfit">User Management</h1>
          <p className="text-sm text-slate-500 font-medium">Verify credentials, toggle suspension, and restrict platform access</p>
        </div>
        ...
        <form onSubmit={handleSearchSubmit} className="relative w-full md:w-80">
          <input
            ...
            className="w-full bg-white border border-slate-200 focus:border-[#005ab3] focus:ring-1 focus:ring-[#005ab3] rounded-2xl py-2.5 pl-10 pr-4 text-sm outline-none transition-all duration-150"
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-50">
                {users.map((u) => (
                  <tr key={u.id} className="hover:bg-slate-50/30 transition-colors duration-100">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <div>
                          <span className="font-bold text-slate-800 text-sm block leading-tight">{u.displayName}</span>
