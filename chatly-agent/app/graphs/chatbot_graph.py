...
def build_chatbot_graph(llm: ChatGroq) -> CompiledStateGraph:
    ...
    No checkpointer is used here — conversation history is injected
    explicitly from MongoDB on every turn by ChatService, so persisting
    state in MemorySaver would cause duplicated messages after turn 1.
    """
